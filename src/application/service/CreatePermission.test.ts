...
describe('CreatePermission', () => {
  ...
  it('throws error if the authenticated user does not have permission to create new permissions', async () => {
    ...
    await expect(
      createPermission.execute('NON_EXISTING_PERMISSION_NAME', user)
    ).rejects.toEqual(new AccessDeniedError(CREATE_NEW_PERMISSION));
  });

  it('creates new permission if the authenticated user has the right permission', async () => {
    const user = await persistedUserWithRoleAndPermissions(ADMIN, [
      CREATE_NEW_PERMISSION,
    ]);
    ...
    const persistedPermission = await permissionRepository.findByName(
      newPermissionName
    );
