...
describe('AssignPermissionToRole', () => {
  ...
  it('throws error if given role is not found', async () => {
    const authenticatedUser = await persistedUserWithRoleAndPermissions(ADMIN, [
      ASSIGN_PERMISSION_TO_ROLE,
    ]);

    await expect(
      assignPermissionToRole.execute(
        ASSIGN_PERMISSION_TO_ROLE,
  ...
  it('throws error if the permission is not found', async () => {
    ...
    ).rejects.toEqual(new PermissionNotFoundError('NON_EXISTING_PERMISSION'));
  });
  ...
  it('throws error if the authenticated user does not have the necessary permission', async () => {
    ...
    const permission = await permissionRepository.save(
      new Permission('NEW_PERMISSION_NAME')
    );

    await expect(
      assignPermissionToRole.execute(permission.name, ADMIN, authenticatedUser)
    ).rejects.toEqual(new AccessDeniedError(ASSIGN_PERMISSION_TO_ROLE));
  });
  ...
  it('works if the authenticatedUser has the right permission', async () => {
    ...
    expect(
      persistedRole?.permissions().map((permission) => permission.name)
