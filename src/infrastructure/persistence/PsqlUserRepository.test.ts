...
describe('PsqlUserRepository', () => {
  ...
  it('inserts new and retrieves a user by id', async () => {
    ...
    expect(persistedUser?.authenticationDetails.method).toBe(AuthenticationMethodType.MAGIC_LINK);
  ...
  it('inserts new and retrieves a user by estonian id', async () => {
    const user = await psqlUserRepository.save(
      User.create(
        new AuthenticationDetails(AuthenticationMethodType.ESTONIAN_ID, new AuthenticationIdentifier('32123'))
      )
    );
