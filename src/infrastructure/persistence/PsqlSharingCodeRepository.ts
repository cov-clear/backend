...
export class PsqlSharingCodeRepository implements SharingCodeRepository {
  ...
  async findByCode(code: string) {
    const linkRow: any = await this.db('sharing_code')
      .select(['code', 'user_id as userId', 'duration', 'creation_time as creationTime'])
      .where('code', '=', code)
      .first();
    ...
    return new SharingCode(new UserId(linkRow.userId), linkRow.code, linkRow.duration, linkRow.creationTime);
  }
  ...
  async save(sharingCode: SharingCode) {
    return await this.db
      .raw(
        `
      insert into sharing_code (code, user_id, duration, creation_time)
      values (:code, :user_id, :duration, :creation_time)
    `,
        {
          code: sharingCode.code,
          user_id: sharingCode.userId.value,
          duration: sharingCode.duration,
